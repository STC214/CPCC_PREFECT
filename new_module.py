...
try:
    ...
    psutil = None
    logging.warning(
        "psutil library not found. Disk space checking will be limited.")
...
class ConfigManager:
    ...
    def edit_interactively(self):
        ...
        while True:
            ...
            print(
                f"4. 跳过步骤确认: {'启用' if self['skip_step_confirmations'] else '禁用'}")
            ...
            if choice == '1':
                new_val = input(
                    f"输入新的起始编号 (当前: {self['start_num']}): ").strip()
                if new_val.isdigit():
                ...
                new_val = input(
                    f"输入压缩级别 (当前: {self.get_compress_level_name()}): ").strip()
                valid_levels = {
...
class DirectoryValidator:
    ...
    def validate_structure(self) -> Tuple[bool, List[str]]:
        ...
        # 检查目录结构
        parent_dirs = [d for d in self.root_dir.iterdir(
        ) if d.is_dir() and d.name not in self.config["skip_dirs"]]
        if not parent_dirs:
        ...
        for parent in parent_dirs:
            ...
            ) and d.name not in self.config["skip_dirs"]]
    ...
    def validate_disk_space(self, required_mb: int = 1000) -> bool:
        ...
        try:
            ...
            if free_mb < required_mb:
                logger.warning(
                    f"磁盘空间不足: 剩余 {free_mb:.1f}MB，需要至少 {required_mb}MB")
                return False
            return True
        except Exception as e:
            logger.warning(f"检查磁盘空间时出错: {e}")
            return True  # 出错时假设空间足够
...
class ProgressManager:
    ...
    def _update_display_ansi(self):
        ...
        # 总进度
        total_progress = (self.current_step - 1 +
                          self.task_progress) / self.total_steps
        sys.stdout.write(
            f"总进度: {self._create_bar(total_progress)}".ljust(width) + "\n")

        # 当前任务
        sys.stdout.write(
            f"任务进度: {self._create_bar(self.task_progress)}".ljust(width) + "\n")

        # 当前步骤
        sys.stdout.write(
            f"执行步骤: {self.current_step}/{self.total_steps}".ljust(width) + "\n")
...
def step4_add_prefix(root: Path, progress: ProgressManager, config: ConfigManager, backup_manager: BackupManager):
    ...
    for idx, subdir in enumerate(subdirs):
        ...
        for file in files:
            ...
            # 统一使用.jpg后缀
            base_name = file.stem + '.jpg'
            new_name = f"{dir_num}_{base_name}"
        ...
        for file in files:
            try:
                ...
                while new_path.exists() and counter <= 100:
                    new_name = f"{file.stem}_{counter}.jpg"  # 统一使用.jpg后缀
        ...
        with tempfile.TemporaryDirectory(dir=str(directory.parent)) as temp_dir:
            ...
            for new_idx, (orig_name, temp_file) in enumerate(temp_files, start=1):
                ...
                try:
                    ...
                    backup_manager.record_operation(
                        "move", temp_file, new_path)
...
def parse_arguments():
    ...
    parser.add_argument('--compress-level', type=str,
                        help='压缩级别 (stored, deflated, bzip2, lzma)')
