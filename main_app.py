...
class MainWindow(QMainWindow):
    ...
    def enable_windows_dark_mode(self):
        """在Windows系统上启用深色标题栏"""
        ...
        try:
            ...
            # 检查是否支持深色模式 (Windows 10 1809+)
            if hasattr(ctypes.windll, 'dwmapi'):
                DWMWA_USE_IMMERSIVE_DARK_MODE = 20
                is_dark = ctypes.c_int(1)
...
def set_windows_app_user_model_id():
    """设置Windows应用用户模型ID，确保任务栏图标正确显示"""
    if sys.platform == "win32":
        try:
            import ctypes
            myappid = 'comic.file.processor.1.0'  # 应用程序ID
